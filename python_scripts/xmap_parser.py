...
def parse_xmap(xmap, cmap):
    ...
    output.write("REF_ID\tQRY_ID\tREF_START\tREF_END\tREF_LEN\tQUERY_START\tQUERY_END\tQUERY_LEN\tGAP_LEN\tRESTR_SITES\tGAP_RESTR_SITES\n")
    ...
    for value in xmap_values:
        ...
        if value.startswith('#'):
            ...
            if float(qry_end_pos) > float(qry_start_pos):
                qry_len = float(qry_end_pos) - float(qry_start_pos)
            else:
                qry_len = float(qry_start_pos) - float(qry_end_pos)
        ...
        # Restriction site info is appended to xmap stats, go through and write to output
            xmap_stats = find_restr_sites(cmap, ref_contig_ID, qry_contig_ID, ref_start_pos, ref_end_pos, ref_len, qry_start_pos, qry_end_pos, qry_len, gap_length)
...
def find_restr_sites(cmap, ref_id, qry_id, ref_start, ref_end, ref_len, qry_start, qry_end, qry_len, gap_length):
    ...
    output_list.append([ref_id, qry_id, ref_start, ref_end, ref_len, qry_start, qry_end, qry_len, gap_length, counter, gap_counter])
    return output_list

###################### Commands #########################

parse_xmap("ref_Nov_17/glazer_chr_scaffolds_wrap_BspQI_to_GAST_ACUL_2014_057_DEFAULT_T_REFINEFINAL1.xmap", "glazer_chr_scaffolds_wrap_BspQI.cmap")
